    ...
    while True:
        ...
        try:
            ...
            if response_json.get("code") == 200:
                print("✅ تم تنفيذ السحب بنجاح!")
                break  # خروج من الحلقة بعد نجاح السحب
            elif response_json.get("code") in [203, 204]:  # انتهاء صلاحية التوكن
                print("🔄 انتهت الجلسة، إعادة تسجيل الدخول...")
                relogin()  # تحديث التوكن ثم إعادة المحاولة
            else:
                print(f"⚠️ فشل السحب، الرد: {response_json}")

        except requests.exceptions.RequestException as e:
            print(f"⚠️ خطأ أثناء تنفيذ السحب: {e}")
        
        time.sleep(15)  # انتظار 5 ثوانٍ قبل إعادة المحاولة
